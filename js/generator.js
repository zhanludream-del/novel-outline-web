class NovelGenerator {
    ...
    composeChapterPrompt({
        ...
        const basePrompt = promptTemplate && promptTemplate.trim()
            ...
            "4. 如果本章出现次日、清晨、午后、傍晚、几日后、当晚等时间变化，必须在文末 STATE_JSON 的 timeline/current_time 写出新的时间点，不能继续沿用上一章时间。",
            "5. 不要把下一章核心事件提前展开，只能做铺垫。",
            "6. 尚未正式见面的角色，不能突然写成熟人互动；模糊称呼尽量改成真实姓名。",
            "7. 如果上一章情绪还没落下，本章开头要延续那股情绪，不要突然换频道。",
            "8. 如果前文交接略生硬，要用动作、对话、场景变化自然补桥，不要生硬跳切。",
            "9. 本章结尾必须停在钩子上，卡在下一章核心事件发生前最有张力的一刻，绝对不能把下一章内容先写进去。",
            "10. 正文写完后，必须按下面协议追加追踪输出。",
            "",
    ...
    buildStateOutputProtocol(project, chapter, relevantCharacters) {
        ...
        return [
            ...
            "<<<STATE_JSON>>>",
            "{",
            '  "timeline": "当前故事时间点（如：次日清晨/三日后傍晚/当夜子时；若本章发生时间变化必须更新）",',
