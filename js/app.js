...
${(detailedOutline || concept || "未填写").slice(0, 2200)}`;
    ...
    detectAiCliche(content) {
        ...
        const phrases = [
            ...
            "病态的占有欲", "极具压迫感", "双目猩红", "布满血丝",
            "气氛降至冰点", "在沉默中拉开序幕"
        ];
        const regexRules = [
            { label: "空泛氛围句", pattern: /(空气|气氛|四周|周围|空间|寝宫内|殿内|殿外)[^。！？\n]{0,18}?(紧绷|凝滞|压抑|死寂|寂静|冷得|升高|令人窒息|背德感|降至冰点)/u },
            { label: "群像氛围句", pattern: /(所有人|众人|满朝文武|群臣)[^。！？\n]{0,18}?(愣住了|吓到了|默不作声|屏住了呼吸|不敢出声|不敢动)/u },
            { label: "群像恐惧句", pattern: /(没人|无人|没有人|没有一个人)[^。！？\n]{0,10}?敢(出声|喝骂|接话|上前|动|阻拦)/u },
            { label: "死寂模板句", pattern: /(死一般的寂静|死一般的沉寂|死寂|鸦雀无声|落针可闻)/u },
            { label: "序幕模板句", pattern: /在沉默中拉开序幕/u },
            { label: "爽文盖章句", pattern: /这哪里是什么[^。！？\n]{0,40}?这分明是/u },
            { label: "概念情绪句", pattern: /(屈辱感|愤怒|杀意|恨意|怒火|恐惧|羞耻感|绝望)[^。！？\n]{0,20}?(翻涌|啃噬|蔓延|席卷|吞没|炸开|疯长)/u },
            { label: "情绪并列句", pattern: /(屈辱|愤怒|杀意|恨意|怒火|恐惧|羞耻)(、|，)(屈辱|愤怒|杀意|恨意|怒火|恐惧|羞耻)/u },
            { label: "情绪生根句", pattern: /这些情绪[^。！？\n]{0,20}?(生根发芽|疯长|翻涌|盘踞)/u },
            { label: "情绪重复句", pattern: /((嫉妒|恨意|怒火|杀意|恐惧|屈辱|占有欲))(?:[，、。！？]\s*|\s+)(?:疯狂的、病态的|疯狂的|病态的)?\s*\1/u },
            { label: "压迫感模板句", pattern: /(极具压迫感|压迫感[^。！？\n]{0,18}?(灌满|席卷|压在|笼罩|扑面而来|逼得)|肃杀而狂暴的压迫感|恐怖杀气)/u },
            { label: "血丝模板句", pattern: /(双目猩红|眼底[^。！？\n]{0,10}?布满血丝|血丝[^。！？\n]{0,10}?像[^。！？\n]{0,10}网)/u },
            { label: "模板推进句", pattern: /(太快了|就是现在|就在这时|下一刻|紧接着|然而|顺势|猛地|毫不犹豫|瞬间|彻底|剧烈|没有再给[^。！？\n]{0,8}机会)/u }
        ];
        const hits = phrases.filter((phrase) => text.includes(phrase));
        regexRules.forEach((rule) => {
