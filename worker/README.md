...
3. 或者把整个 `worker` 文件夹拉到本地，用 `wrangler.toml.example` 改名成 `wrangler.toml`
4. 执行部署
```bash
...
Worker 会根据你在脑洞生成器输入的关键词，尽量去匹配最相关的榜单分类。
...
它会优先尝试匹配带这些词的番茄榜单分类，再抓对应分类前几名样本。
...
- 番茄部分榜单页会对书名做字体混淆，测试版接口会尽量避开直接展示乱码书名，优先输出分类和趋势摘要
